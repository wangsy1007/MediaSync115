...
class FeiniuMediaIndex(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "media_type", "tmdb_id", name="uq_feiniu_media_index_media_tmdb"
        ),
        Index("ix_feiniu_media_index_media_type_tmdb", "media_type", "tmdb_id"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    media_type: Mapped[str] = mapped_column(String(10), nullable=False)
    tmdb_id: Mapped[int] = mapped_column(Integer, nullable=False)
    feiniu_item_ids_json: Mapped[str] = mapped_column(
...
class FeiniuTvEpisodeIndex(Base):
    __tablename__ = "feiniu_tv_episode_index"
    __table_args__ = (
        UniqueConstraint(
            "tmdb_id",
            "season_number",
            "episode_number",
            name="uq_feiniu_tv_episode_index_episode",
        ),
        Index("ix_feiniu_tv_episode_index_tmdb", "tmdb_id"),
    )
...
class FeiniuSyncState(Base):
    __tablename__ = "feiniu_sync_state"
