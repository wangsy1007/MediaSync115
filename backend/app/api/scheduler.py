...
async def run_scheduler_job(job_id: str, force: bool = False):
    job_label = scheduler_manager.resolve_job_label(job_id)
    await operation_log_service.log_background_event(
        source_type="api", module="scheduler",
        action="scheduler.job.run_manual", status="info",
        message=f"手动执行定时任务：{job_label}（force={force}）",
        extra={"job_id": job_id, "force": force, **scheduler_manager._job_log_extra(job_id)},
    )
