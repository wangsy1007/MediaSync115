...
当前提供 `latest` 和明确版本号 tag，例如 `1.3.0`；多架构镜像支持 linux/amd64 和 linux/arm64，Docker 客户端会按宿主机平台自动选择对应版本。
...
- 使用固定版本 tag，例如 `1.3.0`：适合想锁版本的用户，但通常不会收到“新版本可更新”提示

## Changelog

`1.3.0` 重点更新：
- 日志页面支持自动刷新
- **ISO 原盘播放**对齐 SmartStrm 302 直链链路，强制本地 Range 反代，根治播放卡顿/退出
